...
pub fn mcfg_brute_force() -> impl Iterator<Item = u64> {
    ...
    let mcfg_brute_force_iter = if cfg!(opt_level = "0") {
        mcfg_brute_force_inner(0x0..0x1000).flatten()
    } else {
        mcfg_brute_force_inner(0x0..0xffff).flatten()
    };

    // Will figure out later how not to hardcode this
    for addr in mcfg_brute_force_iter {
